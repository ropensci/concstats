...
test_that("concstats_top_df function operates properly", {
  ...
  expect_equal(concstats_top_df(test_df2, na.rm = FALSE), NA_real_)
  ...
  #'  unsupported types
  expect_error(concstats_top_df(test_dfch, !is.numeric(test_dfch$y)))
  #' @srrstats {G5.2, G5.2a, G5.2b, G5.8c} Error on vector with all-`NA` fields
  expect_error(concstats_top_df(test_df9, na.rm = TRUE, class = data.frame()))
  expect_error(concstats_top_df(test_df2, na.rm = FALSE|TRUE))
  expect_error(concstats_top_df(test_df, na.rm = 0))
  expect_error(concstats_top_df(test_df1, as.logical(any(x < 0))))
...
test_that("concstats_top_df returns top market share", {

  x4 <- c(20, 25, 40, 10, 5)
  id <- c(1, 2, 3, 4, 5)
  test_df4 <- data.frame(id, x4)
  top_df <- concstats_top_df(test_df4, "x4")
