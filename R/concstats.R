...
#' market structure and concentration measures, e.g. k-firm ratios, HHI, dual of HHI,
#' Stenbacka index, Entropy and others.
#'
...
#' @note the vector of market shares should be in a decimal format corresponding to total
#'     share of indivudual firms/units (e.g. share <- c(0.35, 0.4, 0.05, 0.1, 0.06, 0.04 )).
...
concstats <- function(x, na.rm = TRUE) {
  ...
  results <- data.frame(measures=c("Firms", "Nrs equivalant", "Top (%)", "Top3 (%)", "Top5 (%)", "HHI", "HHI(min)", "HHI(dual)", "Dominance index",
